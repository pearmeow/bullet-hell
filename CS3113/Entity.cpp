...
void Entity::checkCollisionX(Map* map) {
    ...
    if (map->isSolidTileAt(rightCentreProbe, &xOverlap, &yOverlap) && mVelocity.x > 0.0f && yOverlap >= 0.5f) {
    ...
    if (map->isSolidTileAt(leftCentreProbe, &xOverlap, &yOverlap) && mVelocity.x < 0.0f && yOverlap >= 0.5f) {
...
void Entity::update(float deltaTime, Entity* player, Map* map, Entity* collidableEntities,
